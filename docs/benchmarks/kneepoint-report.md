...
## 追加：两级缓存（L1 Caffeine）效果验证（同日）

拐点定位的瓶颈链里「每次缓存命中仍走 4 次 Redis RTT」是首要开销，据此在详情链路加入 L1 本地缓存（500 条 / 60s TTL / 写路径同点失效，计数与点赞状态仍实时合并）：

- 微基准（JMH，`raw/jmh-l1-2026-09-24.json`）：L1 命中 **51 ns/op** vs Redis L2 命中 **613.7 µs/op（≈12,000×）**；
- 端到端复测（同驱动、同 8 个热点 id、view@100 并发 30s）：**TPS 229.7 → 354.5（+54.3%）**，p50 270.0ms → 163.5ms（`raw/knee_l1_view_100.json`）。
- 诚实边界：单实例进程内缓存，多实例部署需要缓存一致性问题（本实验未覆盖）；60s TTL 内内容字段可能滞后（写路径失效兜底）。
